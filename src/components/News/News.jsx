...
import { selectGetKeyword, selectGetNews, selectLoadingSuccess, selectPaginator } from "../../redux/newsSelectors";
...
const News = () => {
  ...
  const news = useSelector(selectGetNews)
    .map((item) => <New key={item.content} title={item.title} description={item.description} img={item.urlToImage} />);
